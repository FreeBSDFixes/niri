...
Patched crate lives in `freebsd/crates/drm-0.14.2/`.

### What goes wrong

The drm crate's `NodeType::from_dev_id` derives the DRM node type from
`minor(st_rdev) >> 6`. That works on Linux because the st_rdev minor
number is the DRM device number. On FreeBSD the two are unrelated:
...
On FreeBSD and DragonFly, implement `from_dev_id` using `devname()` to
read the kernel's name for the device and parse the trailing integer
out of strings like `drm/0` or `drm/128`. The result is then shifted by
6 as on Linux to determine the node type.

Also fix `has_render` which was hardcoded to `false` on FreeBSD. With
the correct node type detection in place it can now use the same logic
as Linux.

### Relationship to the ports' patch-drm

The FreeBSD ports tree has a `patch-drm` for drm 0.14.1 that predates
upstream PR #210. drm 0.14.2 contains PR #210, so the ports patch no
longer applies as-is. However PR #210 only restructured the code and
did not fix the underlying `minor` vs device number issue, so drm
0.14.2 is still broken on FreeBSD. Our patch replaces both.

### If the drm crate version changes

`freebsd/apply.sh` is pinned to `drm-0.14.2`. When niri bumps to a
newer drm, delete `freebsd/crates/drm-X.Y.Z/`, update the setup_crate
call in `apply.sh`, and adjust the patch if the surrounding code
shifted. Sanity check: before shipping, grep the new source for
`fn from_dev_id` and make sure the FreeBSD branch uses devname().
